...
static void print_all_registers(void) {
    char buf[64];
    putStr("Reg | Value      | Addr\r\n");
    putStr("-------------------------------\r\n");
    int i;
    for (i = 0; i < NUM_REGISTERS; ++i) {
        snprintf(buf, sizeof(buf), "R%-2d | %-10ld | 0x%08lX\r\n",
                 i, (long)registers[i], (unsigned long)(uintptr_t)&registers[i]);
        putStr(buf);
        // Short delay can help if UART hangs (optional): for (volatile int d=0; d<8000; ++d);
    }
}
