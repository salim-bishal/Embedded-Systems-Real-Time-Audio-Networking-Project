...
static void handleLine(char *line)
{
    ...
    if(!cmd) return;
    if(*cmd!='-'){ errorCount[ERR_UNKNOWN_CMD]++; putStr("?? nknown (expected leading '-')\r\n"); return; }
    cmd++;
...
void *mainThread(void *arg0)
{
    ...
    p.readTimeout   = 1;           /* ← non-blocking poll */
    ...
    for(;;){
        ...
        if(n==0) continue;      /* no byte — loop again */
